var conversionElement = {
    ...
    initialize: function() {
        ...
        var nodePanel = new ConversionPanel($(panelContainer));

        nodePanel.setupPanel();
        
        // Setup model update on change
        var panelDiv = $(panelContainer).find("[name='conversionDiv']");
        $(panelDiv).data("model", this.model);
        $(panelDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var panelDiv = $(event.target).closest("[name='conversionDiv']");
