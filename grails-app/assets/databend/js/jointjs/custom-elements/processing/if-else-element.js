var ifElseElement = {
    ...
    template:
        ...
        "   <label class='nodeDescription'>If/Else</label>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>If/Else</span>" + 
	    "		<br>" + 
	    "		<table><tr>" +	
		"			<td><label>If/Else Condition: </label></td> <td><div class='ifElseCondition aceField'></div></td>" +
		"		</tr></table>" +
	    "	</div>" +
        "</div></div>",
    ...
    initialize: function() {
        ...
        // Setup the if/else condition
		aceEdit = ace.edit( this.$box.find(".ifElseCondition").get(0) );
		aceEdit.setTheme("ace/theme/chrome");
		aceEdit.getSession().setMode("ace/mode/runtimeboolean");
		aceEdit.setOptions({
			fontSize: "1em",
			showPrintMargin: false
		});
		this.$box.find(".ifElseCondition").data("ace", aceEdit);
        
        // Reacting on the identifiers & comparator change and store the identifier data in the cell model.
		var object = this;
        this.$box.find('.ifElseCondition').data("ace").getSession().on('change', _.bind(function(evt) {
            object.model.set('ifElseCondition', object.$box.find('.ifElseCondition').data("ace").getValue());
        }, this));
        // Load the trigger identifier & comparator with the value stored in the model
       if(this.model.get('ifElseCondition') != null){ 
    	   this.$box.find('.ifElseCondition').data("ace").setValue(this.model.get('ifElseCondition'));
       }
