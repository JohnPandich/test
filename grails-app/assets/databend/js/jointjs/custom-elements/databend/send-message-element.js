var sendMessageElement = {
    template:
        "<div class='node'><div class='nodeEdge'>" +
        "   <span class='nodeDescription'>Send Message</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Send Message</span>" + 
	    "		<br>" + 
    ...
    initialize: function() {
        ...
        var nodePanel = new SendMessagePanel(panelContainer, messageURL, connectionURL, null);
        nodePanel.openSendPanel();
        
        // Setup model update on change
        var sendMessageDiv = $(panelContainer).find("[name='sendMessageDiv']");
        $(sendMessageDiv).data("model", this.model);
        $(sendMessageDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var sendMessageDiv = $(event.target).closest("[name='sendMessageDiv']");
        	$(sendMessageDiv).data("model").set('nodeData', nodePanel.getDataFunction());
        });
