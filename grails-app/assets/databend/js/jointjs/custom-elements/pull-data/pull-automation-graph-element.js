var pullAutomationGraphElement = {
    template:
        "<div class='node'><div class='nodeEdge'>" +
        "   <span class='nodeDescription'>Pull Automation Graph</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Pull Automation Graph</span>" + 
	    "		<br>" + 
    ...
    initialize: function() {
        ...
        var nodePanel = new PullDataPanel($(panelContainer), templateURL, analysisURL, automationURL, messageURL, dataScriptURL);

        nodePanel.setupPullAutomationPanel();
        
        // Setup model update on change
        var dataDiv = $(panelContainer).find("[name='pullDataDiv']");
        $(dataDiv).data("model", this.model);
        $(dataDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var dataDiv = $(event.target).closest("[name='pullDataDiv']");
        	$(dataDiv).data("model").set('nodeData', nodePanel.getDataFunction($(dataDiv)));
        });
        
        $(dataDiv).find().change( function(){
        	this.model.set('nodeData', nodePanel.getDataFunction($(dataDiv)));
        });
        // Initialize panel data
        var setDataFunction  = $(dataDiv).data("setDataFunction");   
        nodePanel.setDataFunction($(dataDiv), this.model.get('nodeData'));
