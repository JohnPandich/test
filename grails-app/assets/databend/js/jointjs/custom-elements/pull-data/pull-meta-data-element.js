var pullMetaDataElement = {
    template:
        "<div class='node'><div class='nodeEdge'>" +
        "   <span class='nodeDescription'>Pull Meta Data</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Pull Meta Data</span>" + 
	    "		<br>" + 
    ...
    initialize: function() {
        ...
        var dataDiv = $(panelContainer).find("[name='getMetaDataDiv']");
    
        $(dataDiv).data("model", this.model);
        $(dataDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var dataDiv = $(event.target).closest("[name='getMetaDataDiv']");
