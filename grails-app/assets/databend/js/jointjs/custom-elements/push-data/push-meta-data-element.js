var pushMetaDataElement = {
    template:
        "<div class='node'><div class='nodeEdge'>" +
        "   <span class='nodeDescription'>Push Meta Data</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Push Meta Data</span>" + 
	    "		<br>" + 
    ...
    initialize: function() {
        ...
        var nodePanel = new GetMetaDataPanel($(panelContainer), connectionURL, connectionTypeURL);
        nodePanel.setupPanel();
        
        // Setup model update on change
        var dataDiv = $(panelContainer).find("[name='setMetaDataDiv']");
        $(dataDiv).data("model", this.model);
        $(dataDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var dataDiv = $(event.target).closest("[name='setMetaDataDiv']");
