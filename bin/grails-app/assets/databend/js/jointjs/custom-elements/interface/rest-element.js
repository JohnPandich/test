var restElement = {
    template:
        ...
        "   <span class='nodeDescription'>ReST Interface</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>ReST Interface</span>" + 
	    "		<br>" + 
    ...
    initialize: function() {
        ...
        var nodePanel = new RestPanel($(panelContainer));
        nodePanel.setupRestPanel();
        
        // Setup model update on change
        var restDiv = $(panelContainer).find("[name='restDiv']");
        $(restDiv).data("model", this.model);
        $(restDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var restDiv = $(event.target).closest("[name='restDiv']");
        	$(restDiv).data("model").set('nodeData', nodePanel.getDataFunction());
        });
