var triggerEventElement = {
    template:
        "<div class='node'><div class='nodeEdge'>" +
        "   <span class='nodeDescription'>Trigger Event</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Trigger Event</span>" + 
	    "		<br>" + 
	    "		<table>" +
		"			<tr><td><label>Event: </label></td> <td class='standardPadding'><span class='event standardBorder clickable standardPadding'>...</span></td></tr>" +
	    "		</table>" +
	    "	</div>" +
        "</div>",
    ...
    initialize: function() {
        ...
        // Reacting on the trigger name change and store the trigger name data in the cell model.
        this.$box.find('.event').on('DOMSubtreeModified', _.bind(function(evt) {       	
            this.model.set('event', $(evt.target).html());
        }, this));
        // Load the trigger name with the value stored in the model
        this.$box.find('.event').html(this.model.get('event'));
		...
		var selectFunction = function (name, classification){		
			...
			this.$box.find('.event').val(fullClass);
		...
		var select = function(){	
			...
			jQuery.ajax({
				...
				success: function(result){
					var selectPanel = new SelectPanel($("#diags"), "Select Event", ["Name"], result, selectFunction, "treeTableEventRep");						
					selectPanel.open(); 					
				},
        ...
        this.$box.find('.event').click(function(event){  	
        	select();
        });
