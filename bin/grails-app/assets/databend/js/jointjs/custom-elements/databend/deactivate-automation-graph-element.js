var deactivateAutomationGraphElement = {
    template:
        "<div class='node'><div class='nodeEdge'>" +
        "   <span class='nodeDescription'>Deactivate Automation Graph</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Deactivate Automation Graph</span>" + 
	    "		<br>" + 
	    "		<table><tr>" +
		"			<td><label>Automation Graph: </label></td> <td class='standardPadding'><span class='automationGraph standardBorder clickable standardPadding'>...</span></td>" +
	    "		</tr></table>" +
	    "	</div>" +
        "</div>",
    ...
    initialize: function() {
		...
		var selectFunction = function (name, classification){		
			...
			this.$box.find('.automationGraph').val(fullClass);
		}
		var select = function(){
