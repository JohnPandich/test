var dereferenceDataElement = {
    template:
        "<div class='node'><div class='nodeEdge'>" +
        "   <span class='nodeDescription'>Dereference Data Pool Variable</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Dereference Data Pool Variable</span>" + 
	    "		<br>" + 
    ...
    initialize: function() {
        ...
        var nodePanel = new DereferenceDataPanel($(panelContainer));
        nodePanel.setupPanel();
        
        // Setup model update on change
        var dereferenceDataDiv = $(panelContainer).find("[name='dereferenceDataDiv']");
        $(dereferenceDataDiv).data("model", this.model);
        $(dereferenceDataDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var dereferenceDataDiv = $(event.target).closest("[name='dereferenceDataDiv']");
        	$(dereferenceDataDiv).data("model").set('nodeData', nodePanel.getDataFunction());
        });
