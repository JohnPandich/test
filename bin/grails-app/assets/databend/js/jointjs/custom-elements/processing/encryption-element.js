var encryptionElement = {
    ...
    initialize: function() {
        ...
        var nodePanel = new EncryptionPanel($(panelContainer));

        nodePanel.setupPanel();
        
        // Setup model update on change
        var panelDiv = $(panelContainer).find("[name='encryptionDiv']");
        $(panelDiv).data("model", this.model);
        $(panelDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var panelDiv = $(event.target).closest("[name='encryptionDiv']");
