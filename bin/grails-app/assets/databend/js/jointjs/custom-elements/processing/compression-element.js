var compressionElement = {
    template:
        ...
        "   <span class='nodeDescription'>Compression</span>" +
        "	<button class='nodeShowHide icon-minus'></button>" +
        "	<button class='nodeDelete icon-cross'></button>" +
	    "	<div class='nodeContent'>" +
	    "		<span class='nodeTitle' contenteditable='true'>Compression</span>" + 
	    "		<br>" + 
    ...
    initialize: function() {
        ...
        var nodePanel = new CompressionPanel($(panelContainer));
        nodePanel.setupPanel();
        
        // Setup model update on change
        var panelDiv = $(panelContainer).find("[name='compressionDiv']");
        $(panelDiv).data("model", this.model);
        $(panelDiv).find("[name]").on('DOMSubtreeModified', function(event){
        	var panelDiv = $(event.target).closest("[name='compressionDiv']");
